...
import {
  ConfirmationAlertEntity,
  MiniAlertConfirmationEntity,
  MiniAlertEntity,
} from "./views/layout/alert/AlertEntity";
...
const BatchCheckIn = lazy(
  () => import("./views/page/batch_check_in/BatchCheckIn")
);
const LinenWashing = lazy(
  () => import("./views/page/linen_washing/LinenWashing")
);
const LinenPacking = lazy(
  () => import("./views/page/linen_packing/LinenPacking")
);
const BatchCheckOut = lazy(
  () => import("./views/page/batch_check_out/BatchCheckOut")
);
...
function App() {
  ...
  const [miniAlertEntity, setMiniAlertEntity] =
    useState<MiniAlertEntity | null>(null);
  const [miniAlertConfirmationEntity, setMiniAlertConfirmationEntity] =
    useState<MiniAlertConfirmationEntity | null>(null);
  const [stateShowConfirmationAlert, setStateShowConfirmationAlert] =
    useState<boolean>(false);
  const [stateShowMiniAlertConfirmation, setStateShowMiniAlertConfirmation] =
    useState<boolean>(false);
  const [stateShowLoading, setStateShowLoading] = useState<boolean>(false);
  const [confirmationAlertEntity, setConfirmationAlertEntity] =
    useState<ConfirmationAlertEntity | null>(null);
  ...
  const contextShowMiniAlertConfirmationFunc = (
    val: MiniAlertConfirmationEntity
  ) => {
